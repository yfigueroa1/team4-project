...
class LoginPage(Screen):
    def __init__(self, **kwargs):
        ...
        logo = Image(source='logo.png', size_hint=(None, None), size=(350, 350), pos_hint={'center_x': 0.5, 'center_y': 0.7})
        ...
        login_button.bind(on_press=lambda instance: self.login_user(username_input.text, password_input.text))
    ...
    def login_user(self, email, password):
        ...
        if self.authenticate(email, hashed_password):
    ...
    def on_remove_quantity_popup_dismiss(self, instance, barcode):       
        if instance.result is not None:            
            ...
            if quantity_to_remove <= current_quantity:                
                ...
                error_popup = Popup(title='Error', content=Label(text='exceeds current quantity.'),
                                    size_hint=(None, None), size=(200, 100))
                error_popup.open()
        else:            
